...
public class NTApplet2D extends PApplet {
	...
	public static final int DEPTH=0, TIME=1;
	public int colorBy;
	public void setup() {
	...
	public void controlEvent(ControlEvent theEvent) {		
		...
		if(theEvent.group().name() == "checkbox"){
			for (int i = 0; i < theEvent.group().arrayValue().length; i++) {
				int state = (int) theEvent.group().arrayValue()[i];
				if (state == 1) { // If that item/box in the array is active
